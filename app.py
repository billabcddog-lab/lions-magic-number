...
if standings and schedule:
    ...
    for team in standings:
        ...
        processed_teams.append({'name': name, 'wins': w, 'losses': l, 'rate': rate, 'orig': team})
        if "攻城獅" in name:
            lions_wins = w
            lions_raw_data = team
    ...
    for i, team_info in enumerate(ranked_teams):
        ...
        if "攻城獅" in name:
            table_data.append({
                "排名": i + 1, "球隊": f"**{name} (我方)**", 
                "勝-敗": f"{team_info['wins']}W - {team_info['losses']}L", "魔術數字": "-"
            })
            continue
            
        table_data.append({
            "排名": i + 1, "球隊": name + status + note,
            "勝-敗": f"{team_info['wins']}W - {team_info['losses']}L",
            "魔術數字": f"M{max(0, m_number)}"
        })

    st.subheader(f"🔥 季後賽晉級/追趕數字：M{max(0, playoff_m)}")
    st.table(table_data)
    st.caption("註：⚖️ 具對戰優勢。⭐ 晉級季後賽的直接競爭對象。")
